...
from imageio import imread                  # membantu membaca gambar
from PyQt5.QtCore import QTimer, Qt
from PyQt5 import QtGui
from tkinter import filedialog
from tkinter import *
import tkinter as tk
from matplotlib import pyplot as plt        # untuk menampilkan gerakan tangan
from matplotlib.widgets import Button
import sys
import os
import cv2                                  # untuk operasi kamera
import numpy as np                          # memproses gambar
import qimage2ndarray                       # konversi gambar ke matrix
import win32api
import winGuiAuto
import win32gui
import win32con
...
index = 0  # index untuk penampil gerakan
engine = pyttsx3.init()
...
image_x, image_y = 64, 64  # resolusi gambar

from keras.models import load_model

classifier = load_model('ASLModel.h5')  # memuat model


def fileSearch():
    """Mencari setiap file yang diakhiri dengan .png di direktori SampleGestures sehingga gerakan kustom dapat diteruskan ke fungsi predictor()"""
    fileEntry = []
...
def load_images_from_folder(folder):
    """Mencari setiap gambar dalam direktori tertentu"""
    images = []
...
def toggle_imagesfwd(event):
    """menampilkan gambar berikutnya yang bertindak sebagai penampil gerakan"""
...
def toggle_imagesrev(event):
    """menampilkan gambar sebelumnya bertindak sebagai penampil gerakan"""
    img = load_images_from_folder('TempGest/')
    global index
...
def openimg():
    """menampilkan gambar gerakan yang telah ditentukan di jendela paling kanan"""
...
def removeFile():
    """Menghapus direktori temp.txt dan temgest jika ada tombol stop yang ditekan atau aplikasi ditutup"""
    try:
        os.remove("temp.txt")
    except:
        pass
...
def clearfunc(cam):
    """mematikan kamera yang dibuka dan memanggil fungsi removeFile()"""
    cam.release()
...
def clearfunc2(cam):
    """mematikan kamera yang dibuka"""
    cam.release()
    cv2.destroyAllWindows()


def saveBuff(self, cam, finalBuffer):
    """Simpan file sebagai temp.txt jika tombol simpan ditekan dalam pembentukan kalimat melalui gui"""
    cam.release()
    cv2.destroyAllWindows()
...
def capture_images(self, cam, saveimg, mask):
    """Menyimpan gambar untuk gerakan khusus jika tombol ditekan dalam pembuatan gerakan khusus melalui gui"""
    cam.release()
    cv2.destroyAllWindows()
...
def predictor():
    """ Bergantung pada model yang dimuat dan prediksi yang disimpan dengan gerakan khusus dibuat dengan memeriksa larik atau melalui algoritma SiFt"""
    import numpy as np
    ...
    for i in range(len(fileEntry)):
        ...
        if (abs(len(good_points) + len(
            ...
            if (gesname == 'sp'):  # sp diganti dengan <spasi>
                gesname = ' '
            return gesname
...
def checkFile():
    """ambil konten temp.txt untuk modul ekspor """
...
class Dashboard(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Setelah semuanya selesai, release
        cap.release()
    ...
    def quitApplication(self):
        """menutup jendela GUI bersama dengan penghapusan file"""
    ...
    def createGest(self):
        """ Modul custom gesture generation """
        try:
            clearfunc(self.cam)
        except:
            pass
    ...
    def exportFile(self):
        """ekspor modul file dengan bantuan tts dan gesture viewer"""
        try:
            clearfunc2(self.cam)
        except:
            pass
    ...
    def on_click(self):
        """Buka jendela tkinter untuk menyimpan file di lokasi yang diinginkan """
        content = checkFile()
        root = Tk()
        root.withdraw()
    ...
    def gestureViewer(self):
        """gesture viewer melalui matplotlib """
        try:
            img = load_images_from_folder('TempGest/')
            plt.imshow(img[index])
        except:
        ...
        plt.show()
        axcut._button = bcut  # membuat referensi untuk elemen tersebut
        axcut1._button1 = bcut1

    # buttonaxe._button = bcut

    def scanSent(self):
        """ modul pembentukan kalimat """
        try:
            clearfunc(self.cam)
        except:
            pass
